...
def extract_and_generate_model(cadence_csv_simu_path, json_path, extModel, display=False, voltageNet=None, OrderedCourbeRef=0):
    ...
    data['k_h'] = [k_h]
    if display:
        plt.title("Normalized activation function, rstd= "+str(round(100*sigma_max,2))+" %")
        plt.plot(YTraceIN, typic_af, 'k', label = "Typic")
        ...
        plt.fill_between(YTraceIN, max_dev, min_dev, color='b',  alpha=.3)
        if (voltageNet == None):
            plt.plot(YTraceIN, sigmoid(YTraceIN/k_h, *popt)*k_h, 'k--', label="Sigmoidal Fitting")
        else:
            plt.plot(YTraceIN, np.poly1d(data['mod'])(YTraceIN/k_h), 'k--', label="Poly Fitting")
        plt.legend()
    ...
    if display:
        ...
        plt.plot(w_hd, np.poly1d(data['g_fct_w'])(w_hd), 'c--')
